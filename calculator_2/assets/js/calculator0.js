$(document).ready(function(){
  ...
  var main_row_balance = 0;
   ...
   $(document).on("click","#calculate",function(){
    ...
    main_payment_of_month = getPayment(ele_amortization_year.value,ele_main_balance.value,ele_monthly_payment.value,ele_int_rate_per.value);
  ...
  var getNextRowInstallment = function(previous_number_of_month,previous_row_balance){
    ...
    main_payment_of_month = getPayment(ele_amortization_year.value,previous_row_balance,ele_monthly_payment.value,ele_int_rate_per.value);
  ...
  function getInterest(main_number_of_month,balance_amount){
    ...
    if(main_number_of_month == 0){
      ...
      col_interest = ((ele_int_rate_per.value*balance_amount)/100).toFixed(2);
    }
    return col_interest;
  }

  function getExtPayment(main_number_of_month,balance_amount){
    var main_ext_payment = 0;
    var plus_with_ext_payment = 0;
    if(parseInt(balance_amount)<=parseInt(ele_monthly_payment.value) || main_number_of_month == 0){
      ...
      if(ele_d18_value.value>0){
        if((main_number_of_month%ele_payment_interval.value) == 0){
          main_ext_payment = ele_d18_value.value;
        }
        else{
          main_ext_payment = 0;
        }
    ...
    return  parseInt(main_ext_payment)+parseInt(plus_with_ext_payment);
  }
...
function get_int_rate_per(interest_rate_value){
  var result = (((1+interest_rate_value/(2*100))**(2/12))-1);
  result = (result*100);
